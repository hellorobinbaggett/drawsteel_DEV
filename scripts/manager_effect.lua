...
function message(sMsg, nodeCTEntry, bGMOnly, sUser)
	...
	if nodeCTEntry then
		msg.text = string.format("%s [on %s]", msg.text, ActorManager.getDisplayName(nodeCTEntry));
	end
...
function decodeEffectFromDrag(draginfo, rTarget)
	...
	elseif sDragType == "number" then
		if sDragDesc:match("%[" .. Interface.getString("action_effect_tag")) then
			bEffectDrag = true;
...
function encodeEffectAsText(rEffect)
	...
	if rEffect then
		table.insert(aMessage, string.format("[%s] %s", Interface.getString("action_effect_tag"), rEffect.sName));
...
function addCondition(rActor, sEffect, bGMOnly)
	if (sEffect or "") == "" then
		return;
	end
